...
fish <- read_csv("data-raw/BlackfootFish.csv")
```
...
fish |> 
  ...
  ggplot() + 
  geom_col(aes(x = year, y = weight), fill = "orchid4") + 
  labs(x = "Year of Survey", y = "Number of Missing Weights") +
  scale_x_continuous(limits = c(1988,2008))
```
...
> When rescaling weight, the code has max weight minus min length instead of min weight.
...
#Judith I'm sorry I cannot do this 
```
